...
object Huffman {
  ...
  // Part 1: Basics
    ...
    def chars(tree: CodeTree): List[Char] = {
      ...
      case Leaf(char,weigth) => List(char)
    } // tree match ...
  ...
  // List[Char] in, List[(Char,Int)] out, unsorted
   ...
   */
  // List[(Char, Int)] in, List[Leaf] out, sorted
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      def order0(sub: List[(Char, Int)],acc: List[Leaf]): List[Leaf] = sub match {
        case Nil     => acc
        case x :: xs => order0( xs,  acc :+ Leaf(x._1, x._2))
      }
      order0(freqs.sortWith((a, b) => a._2 < b._2),List())
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decode0(tree0: CodeTree, bits0: List[Bit], accu: List[Char]): List[Char] = bits0 match {
      ...
      case x :: xs => tree0 match {
        case l: Leaf => decode0(tree, bits0, accu :+ l.char) // this one has to be tree and bit0, not 'tree0' or bits0.tail.
